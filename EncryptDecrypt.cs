...
namespace SampleProject
{
    public class SampleDemoClass
    {
        static void Main(string[] args)
        {
            ...
            var codedWord = Encrypt(Console.ReadLine());
            Console.WriteLine("Encrypted:: " + codedWord);
            var deCodedWord = Decrypt(codedWord);
            Console.WriteLine("Decrypted:: "+ deCodedWord);
            Console.ReadLine();
        }

        public static string Key = "RandomKeyHere";
        public static string Encrypt(string textValue, string encryptKey = null)
        {
            if (string.IsNullOrEmpty(encryptKey))
            {
                encryptKey = Key;//ConfigurationManager.AppSettings["EncryptDecryptKey"];
            }
            if (textValue != null)
            {
        ...
        public static string Decrypt(string encodedText, string decryptKey = null)
        {
            ...
            {
                decryptKey = Key; //ConfigurationManager.AppSettings["EncryptDecryptKey"];
            }
            if (encodedText != null)
            {
                string passPhrase = "Pas5pr@se";
